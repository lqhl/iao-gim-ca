...
void GoUctBoard::CheckConsistencyBlock(SgPoint point) const
{
    ...
    poco_assert(anchorFound);
    poco_assert(color == block->m_color);
    poco_assert(stones.SameElements(block->m_stones));
    poco_assert(liberties.SameElements(block->m_liberties));
...
void GoUctBoard::AddStoneToBlock(SgPoint p, Block* block)
{
    ...
    poco_assert(IsColor(p, block->m_color));
...
{
    // Stone already placed
    poco_assert(IsColor(p, adjBlocks[0]->m_color));
    poco_assert(NumNeighbors(p, adjBlocks[0]->m_color) > 1);
...
{
    poco_assert(p >= 0); // No special move, see SgMove
    poco_assert(p == SG_PASS || (IsValidPoint(p) && IsEmpty(p)));
    CheckConsistency();
    ...
    if (p != SG_PASS)
    {
        ...
        if (m_koPoint != SG_NULLPOINT)
            if (NumStones(p) > 1 || NumLiberties(p) > 1)
                m_koPoint = SG_NULLPOINT;
        poco_assert(HasLiberties(p)); // Suicide not supported by GoUctBoard
    }
