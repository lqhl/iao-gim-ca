...
public:
    ...
    /** initializes the board with new size. */
    void Init(int size);
...
private:
    ...
    public:
        ...
        void print(ostream& out) {
        	out << "Block color: " << SgBW(m_color) << endl;
        	for (int i = 0; i < m_stones.Length(); i++)
        		out << SgPointUtil::PointToString(m_stones[i]) << endl;
        }
    ...
    /** See CountPlay */
    unsigned long long m_countPlay;
...
inline GoBoard::StoneIterator::StoneIterator(const GoBoard& bd, SgPoint p)
    : m_it(bd.m_state.m_block[p]->Stones()),
      m_board(bd)
{
    poco_assert(m_board.Occupied(p));
}
...
inline GoBoard::Iterator::Iterator(const GoBoard& bd)
    : SgPointRangeIterator(bd.m_boardIter,
                           bd.m_boardIterEnd)
{
}
...
{
    poco_assert(Occupied(anchor));
    poco_assert(Anchor(anchor) == anchor);
