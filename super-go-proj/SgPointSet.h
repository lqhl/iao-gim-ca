...
private:
    ...
    public:
        ...
        {
            poco_assert(boardSize >= SG_MIN_SIZE);
            poco_assert(boardSize <= SG_MAX_SIZE);
...
{
    poco_assert(p < static_cast<int>(m_a.size()));
    m_a.flip(p);
    return (*this);
}
...
inline SgPoint SgSetIterator::operator*() const
{
    ...
    poco_assert(m_set.m_a.test(m_index));
    return m_index;
}
