...
void AddNeighborBlocksOfColor(const GoBoard& bd, SgPoint p, SgBlackWhite color,
...
void AddWall(GoBoard& bd, SgBlackWhite color, SgPoint start, int length,
		int direction);
...
void BlocksAdjacentToPoints(const GoBoard& bd, const SgVector<SgPoint>& points,
...
SgPointArray<int> CfgDistance(const GoBoard& bd, SgPoint p, int maxDist =
...
bool IsSimpleEyeOfBlock(const GoBoard& bd, SgPoint lib, SgPoint blockAnchor,
...
template<class BOARD>
float ScoreSimpleEndPosition(const BOARD& bd, float komi, const SgBWSet& safe,
		bool noCheck, SgPointArray<SgEmptyBlackWhite>* scoreBoard);
...
float ScoreSimpleEndPosition(const GoBoard& bd, float komi, bool noCheck =
		false);
...
template<class BOARD>
float TrompTaylorScore(const BOARD& bd, float komi, SgPointArray<
		SgEmptyBlackWhite>* scoreBoard = 0);
...
inline bool GoBoardUtil::ContainsAnchor(const SgPoint anchor[], const SgPoint p) {
...
inline bool GoBoardUtil::IsNeighborOfSome(const BOARD& bd, SgPoint p,
		SgPoint anchors[], SgBlackWhite toPlay) {
...
bool GoBoardUtil::IsSimpleChain(const BOARD& bd, SgPoint block, SgPoint& other) {
	...
	for (int i = 0; anchors1[i] != SG_ENDPOINT; ++i) {
		...
		if (anchor != block && GoBoardUtil::ContainsAnchor(anchors2, anchor)) {
			other = anchor;
			return true;
		}
	}
...
SgEmptyBlackWhite GoBoardUtil::ScorePoint(const BOARD& bd, SgPoint p,
	...
	if (bd.NumNeighbors(p, SG_BLACK) > 0 && bd.NumNeighbors(p, SG_WHITE) == 0)
		return SG_BLACK;
	else if (bd.NumNeighbors(p, SG_WHITE) > 0 && bd.NumNeighbors(p, SG_BLACK)
			== 0) {
	...
	for (SgNb4Iterator it(p); it; ++it) {
		...
		} else if (nbColor == toPlay) // own stones
		{
			if (bd.NumLiberties(nb) > 2)
				...
				// check block's liberties other than p
...
template<class BOARD>
float GoBoardUtil::TrompTaylorScore(const BOARD& bd, float komi, SgPointArray<
		SgEmptyBlackWhite>* scoreBoard) {
	float score = -komi;
...
/** Iterate through the anchors of all the blocks adjacent to the given
 point. */
class GoNeighborBlockIterator: public SgPointIterator {
public:
...
/** Iterate through the anchors of all the blocks adjacent to the given
 block. */
template<class BOARD>
class GoAdjBlockIterator: public SgPointIterator {
public:
...
template<class BOARD>
GoAdjBlockIterator<BOARD>::GoAdjBlockIterator(const BOARD& board, SgPoint p,
		int maxLib) :
	SgPointIterator(m_points) {
