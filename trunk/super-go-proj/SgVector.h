...
public:
    ...
    void PushBack(const T& elt)
    {
        ...
        poco_assert(Length() < std::numeric_limits<int>::max());
        m_vec.push_back(elt);
    }
    ...
    const T& TopNth(int index) const
    {
        poco_assert(NonEmpty());
        poco_assert(index >= 1);
        poco_assert(index <= static_cast<int>(m_vec.size()));
...
{
    poco_assert(IsSortedAndUnique());
    poco_assert(vector.IsSortedAndUnique());
